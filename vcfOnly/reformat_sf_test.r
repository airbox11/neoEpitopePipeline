...
output_s3 <- args[5]
  ...
  workDir <- '/omics/groups/OE0422/internal/yanhong/all_in_one_pipeline_collection/mhc4.1/DS71/3_add_expression'
  ...
  output_sf <- '/omics/groups/OE0422/internal/yanhong/all_in_one_pipeline_collection/mhc4.1/DS71/3_add_expression/fpkm_tpm.featureCounts.tsv'
  input_s2 <- '/omics/groups/OE0422/internal/yanhong/all_in_one_pipeline_collection/mhc4.1/DS71/2_SNVs_based_neoepitope_prediction/netMHCpan4_1/results_MHCI_epitopes.tab_splitGenes'
  output_s3 <- '/omics/groups/OE0422/internal/yanhong/all_in_one_pipeline_collection/mhc4.1/DS71/3_add_expression/MHCI_epitopes_RNAseq_netMHCpan4_1.tab'
...
##
df4 <- read.table(output_sf, sep = '\t', stringsAsFactors = FALSE, header = TRUE)  
names(df4)[names(df4) == "ensembl_gene_id"] = "geneID"

df4$geneID <- str_match(df4$geneID, pattern = 'ENSG\\d+')
## get geneIDs ====
