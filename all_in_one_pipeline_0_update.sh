...
function f_vcfOnly_origin () {
	...
	findx=`find ${workDir}/2_SNVs_based_neoepitope_prediction -name "*somatic*.vcf" | wc -l`
	if [ $findx == 1 ]; then
	...
	rm -rf 1.vcf
	cat /omics/groups/OE0422/internal/yanhong/all_in_one_pipeline/vcf_header.txt > $workDir/2_SNVs_based_neoepitope_prediction/1.vcf
...
function addRNAseq_origin () {
	...
	if [[ ${vcfOnly,,} == 'promise' ]];then
		vcf=$workDir/2_SNVs_based_neoepitope_prediction/2.vcf
		script_py=/omics/groups/OE0422/internal/yanhong/all_in_one_pipeline/hg38_to_hg19_vcf.py
	...
	if [ ! $hlaID = 'promise' ]; then
		...
		script_col6a3=/omics/groups/OE0422/internal/yanhong/git/inka/exon6_col6a3_FPKM/FPKM_calculation.sh
